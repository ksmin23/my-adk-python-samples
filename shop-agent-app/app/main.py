...
def query_vertex_agent(user_query, user_id, session_id):
  ...
  print(f"Querying Vertex AI agent for user '{user_id}' in session '{session_id}': '{user_query}'...")
        ...
        if 'function_response' in part:
          ...
          if function_response.get('name') == 'find_shopping_items':
            try:
              ...
              print(f"Error parsing items from function_response: {e}")
  ...
  print(f"Recommended products: {len(unique_products)} items")
...
with gr.Blocks(theme=gr.themes.Soft(), title="AI Shopping Assistant") as demo:
  session_state = gr.State({})  # 세션 상태를 저장할 State 객체
...
if __name__ == "__main__":
  print(f"[INFO] Connecting to Vertex AI Agent Engine: {AGENT_ENGINE_ID}")
