...
"""Deployment script for the RAG with Vector Search and Datastore Agent."""
...
try:
  ...
  from rag_with_vectorsearch_ds.agent import root_agent
...
def create_agent(project_id, location, bucket):
  ...
  agent_env_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', 'rag_with_vectorsearch_ds', '.env')
  )
  ...
  if not os.path.exists(agent_env_path):
    ...
    logging.error("Please copy 'rag_with_vectorsearch_ds/.env.example' to 'rag_with_vectorsearch_ds/.env' and fill in the values.")
    sys.exit(1)
  ...
  # Filter for required variables and ensure all are present
  required_vars = [
    "VECTOR_SEARCH_INDEX_ID", "VECTOR_SEARCH_ENDPOINT_ID", "DATASTORE_DATABASE", "DATASTORE_KIND"
  ]

  # We only want to pass these specific variables to the agent
  env_vars_to_pass = {k: v for k, v in env_config.items() if k.startswith("VECTOR_SEARCH_") or k.startswith("DATASTORE_")}

  # Check for missing keys
  missing_keys = [key for key in required_vars if key not in env_vars_to_pass or not env_vars_to_pass[key]]
  if missing_keys:
  ...
  remote_agent = agent_engines.create(
    ...
    extra_packages=["rag_with_vectorsearch_ds"],
...
def main():
  ...
  parser = argparse.ArgumentParser(description="Deploy and manage the RAG with Vector Search and Datastore Agent on Vertex AI Agent Engine.")
