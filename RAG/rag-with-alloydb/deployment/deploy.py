...
"""Deployment script for the RAG with AlloyDB Agent."""
...
try:
  ...
  from rag_with_alloydb.agent import root_agent
...
def create_agent(project_id, location, bucket):
  ...
  # The deploy script is in 'deployment/', the agent code is in 'rag_with_alloydb/'
  agent_env_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', 'rag_with_alloydb', '.env')
  )
  ...
  if not os.path.exists(agent_env_path):
    ...
    logging.error("Please copy 'rag_with_alloydb/.env.example' to 'rag_with_alloydb/.env' and fill in the values.")
    sys.exit(1)
  ...
  # Filter for ALLOYDB_ variables and ensure all required ones are present
  required_vars = [
    "ALLOYDB_REGION", "ALLOYDB_CLUSTER", "ALLOYDB_INSTANCE", 
    "ALLOYDB_DATABASE", "ALLOYDB_USER", "ALLOYDB_PASS", "ALLOYDB_TABLE_NAME"
  ]

  # We only want to pass the ALLOYDB variables to the agent
  alloydb_env_vars = {k: v for k, v in env_config.items() if k.startswith("ALLOYDB_")}

  # Check for missing keys
  missing_keys = [key for key in required_vars if key not in alloydb_env_vars or not alloydb_env_vars[key]]
  if missing_keys:
  ...
  remote_agent = agent_engines.create(
    ...
    env_vars=alloydb_env_vars,
    extra_packages=["rag_with_alloydb"],
...
def main():
  ...
  parser = argparse.ArgumentParser(description="Deploy and manage the RAG with AlloyDB Agent on Vertex AI Agent Engine.")
