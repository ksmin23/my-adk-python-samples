...
"""Deployment script for the RAG with Vector Search Agent."""
...
try:
  ...
  from rag_with_vectorsearch_gcs.agent import root_agent
...
def create_agent(project_id, location, bucket):
  ...
  agent_env_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', 'rag_with_vectorsearch_gcs', '.env')
  )
  ...
  if not os.path.exists(agent_env_path):
    ...
    logging.error("Please copy 'rag_with_vectorsearch_gcs/.env.example' to 'rag_with_vectorsearch_gcs/.env' and fill in the values.")
    sys.exit(1)
  ...
  # Filter for VECTOR_SEARCH_ variables and ensure all required ones are present
  required_vars = [
    "BUCKET_NAME", "VECTOR_SEARCH_INDEX_ID", "VECTOR_SEARCH_ENDPOINT_ID"
  ]

  # We only want to pass the VECTOR_SEARCH variables to the agent
  vector_search_env_vars = {k: v for k, v in env_config.items() if k.startswith("VECTOR_SEARCH_") or k == "BUCKET_NAME"}

  # Check for missing keys
  missing_keys = [key for key in required_vars if key not in vector_search_env_vars or not vector_search_env_vars[key]]
  if missing_keys:
  ...
  remote_agent = agent_engines.create(
    ...
    env_vars=vector_search_env_vars,
    extra_packages=["rag_with_vectorsearch_gcs"],
...
def main():
  ...
  parser = argparse.ArgumentParser(description="Deploy and manage the RAG with Vector Search Agent on Vertex AI Agent Engine.")
