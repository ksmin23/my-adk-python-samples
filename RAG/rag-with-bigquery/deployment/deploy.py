...
"""Deployment script for the RAG with BigQuery Agent."""
...
try:
  ...
  from rag_with_bigquery.agent import root_agent
...
def create_agent(project_id, location, bucket):
  ...
  agent_env_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', 'rag_with_bigquery', '.env')
  )
  ...
  if not os.path.exists(agent_env_path):
    ...
    logging.error("Please copy 'rag_with_bigquery/.env.example' to 'rag_with_bigquery/.env' and fill in the values.")
    sys.exit(1)
  ...
  # Filter for BIGQUERY_ variables and ensure all required ones are present
  required_vars = [
    "BIGQUERY_LOCATION", "BIGQUERY_DATASET", "BIGQUERY_TABLE"
  ]

  # We only want to pass the BIGQUERY variables to the agent
  bigquery_env_vars = {k: v for k, v in env_config.items() if k.startswith("BIGQUERY_")}

  # Check for missing keys
  missing_keys = [key for key in required_vars if key not in bigquery_env_vars or not bigquery_env_vars[key]]
  if missing_keys:
  ...
  remote_agent = agent_engines.create(
    ...
    requirements=[
      ...
      "langchain-google-community[featurestore,bigquery]==2.0.7",
    ...
    env_vars=bigquery_env_vars,
    extra_packages=["rag_with_bigquery"],
...
def main():
  ...
  parser = argparse.ArgumentParser(description="Deploy and manage the RAG with BigQuery Agent on Vertex AI Agent Engine.")
