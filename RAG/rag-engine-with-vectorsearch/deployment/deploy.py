...
"""Deployment script for the RAG Engine with Vector Search Agent."""
...
try:
  ...
  from rag_engine_with_vectorsearch.agent import root_agent
  from vertexai import agent_engines
  from vertexai.preview.reasoning_engines import AdkApp
except ImportError as e:
  print(f"ERROR: Failed to import necessary libraries. {e}", file=sys.stderr)
  print("Please ensure you have installed the necessary dependencies for deployment, such as 'google-cloud-aiplatform[agent_engines,rag]>=1.108.0', 'google-adk>=1.10.0', and 'python-dotenv'.", file=sys.stderr)
  sys.exit(1)
...
def create_agent(project_id, location, bucket):
  ...
  agent_env_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', 'rag_engine_with_vectorsearch', '.env')
  )
  ...
  if not os.path.exists(agent_env_path):
    ...
    logging.error("Please copy 'rag_engine_with_vectorsearch/.env.example' to 'rag_engine_with_vectorsearch/.env' and fill in the values.")
    sys.exit(1)
  ...
  # Filter for RAG_CORPUS and ensure it is present
  required_vars = ["RAG_CORPUS"]

  # We only want to pass the RAG_CORPUS variable to the agent
  rag_env_vars = {k: v for k, v in env_config.items() if k in required_vars}

  # Check for missing keys
  missing_keys = [key for key in required_vars if key not in rag_env_vars or not rag_env_vars[key]]
  if missing_keys:
  ...
  remote_agent = agent_engines.create(
    ...
    requirements=[
      "google-adk==1.10.0",
      "python-dotenv==1.0.1",
      "google-cloud-aiplatform[rag]==1.108.0",
      "google-auth==2.40.3",
      "llama-index==0.13.1",
    ],
    env_vars=rag_env_vars,
    extra_packages=["rag_engine_with_vectorsearch"],
...
def main():
  ...
  parser = argparse.ArgumentParser(description="Deploy and manage the RAG Engine with Vector Search Agent on Vertex AI Agent Engine.")
