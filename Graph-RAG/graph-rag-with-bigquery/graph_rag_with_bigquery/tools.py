...
import os
import logging
from langchain_bigquery_graph import BigQueryGraphStore, BigQueryGraphVectorContextRetriever
...
def _get_graph_store():
    ...
    if _graph_store is None:
        _graph_store = BigQueryGraphStore(
            project_id=os.environ["GOOGLE_CLOUD_PROJECT"],
            dataset_id=os.environ["BIGQUERY_DATASET"],
            graph_name=os.environ["BIGQUERY_GRAPH_NAME"],
...
def retrieve_graph_context_function(query: str) -> str:
    """
    Retrieves relevant context from the BigQuery Graph database based on a query.
